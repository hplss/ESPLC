...
class OutputOBJ: public Ladder_OBJ_Logical
{
	...
	OutputOBJ( const String &id, uint8_t pin, OBJ_TYPE type = OBJ_TYPE::TYPE_OUTPUT, uint8_t logic = LOGIC_NO, uint8_t pwm_channel = 0, uint16_t duty_cycle = 0, double pwm_frequency = -1, uint8_t pwm_resolution = 12 ) 
	: Ladder_OBJ_Logical(id, type), iPin(pin), iPWMChannel(pwm_channel), iLogic(logic)
	{ 
	...
	~OutputOBJ()
	{
		 ...
		 if (iType == OBJ_TYPE::TYPE_OUTPUT_PWM)
